...
void ZMap::LoadPaletteInfo(int terrain_type)
{
    ...
    if(ret != MAX_PLANET_TILES)
        printf("unable to fully load:%s (loaded %u tiles)\n", filename.c_str(),
...
void ZMap::DoRender(SDL_Surface *dest, int shift_x_dest, int shift_y_dest)
{
    ...
    from_rect.x = static_cast<short>(shift_x);
    from_rect.y = static_cast<short>(shift_y);
    from_rect.w = static_cast<Uint16>(view_w);
    from_rect.h = static_cast<Uint16>(view_h);

    to_rect.x = static_cast<short>(shift_x_dest);
    to_rect.y = static_cast<short>(shift_y_dest);
...
int ZMap::Read(char* data, int size, bool scrap_data)
{
    ...
    if(size < static_cast<int>(item_size))
        return 0;
    ...
    for(i=0;i<basic_info.zone_count;i++)
    {
        ...
        if(size < static_cast<int>(item_size))  return 0;
...
int ZMap::Read(const char* filename)
{
    ...
    while( (ret = fread(buf, 1, buf_size, fp)) ) //(ret = fread(buf, 1, buf_size, fp))
    {
        //resize
        if(!map_data)
            map_data = static_cast<char*>(malloc(ret));
        else
            map_data = static_cast<char*>(realloc(
                        map_data, static_cast<size_t>(map_data_size) + ret
                        ));
                      ...
                      SDL_Rect &to_rect)
{
    ...
    //setup from
    from_rect.x = static_cast<Sint16>(shift_x - x);
    from_rect.y = static_cast<Sint16>(shift_y - y);

    if(to_rect.x + src->w > view_w)
        from_rect.w = static_cast<Uint16>(view_w - to_rect.x);
    else
        from_rect.w = static_cast<Uint16>(to_rect.x - view_w);

    if(to_rect.y + src->h > view_h)
        from_rect.h = static_cast<Uint16>(view_h - to_rect.y);
    else
        from_rect.h = static_cast<Uint16>(to_rect.y - view_h);
