...
void ZServer::Run()
{
	...
	while(allow_run)
	{
        whole_time = COMMON::current_time();
...
// 		printf("zserver\n");
		...
		//run stuff
        process_objects_time = COMMON::current_time();
		ProcessObjects();
        process_objects_time = COMMON::current_time() - process_objects_time;
        ...
        whole_time = COMMON::current_time() - whole_time;
...
void ZServer::ProcessObjects()
{
    ...
    delete_time = COMMON::current_time();
    ...
    delete_time = COMMON::current_time() - delete_time;

    process_time = COMMON::current_time();
	//process all the objects
	for(obj=object_list.begin(); obj!=object_list.end();obj++) 
		(*obj)->ProcessServer(zmap, ols);
    process_time = COMMON::current_time() - process_time;

	//see if any pathfinding responses have come back
	//and process them
    path_time = COMMON::current_time();
	ProcessPathFindingResults();
    path_time = COMMON::current_time() - path_time;
    ...
    update_time = COMMON::current_time() - update_time;
