...
void GMMWList::RenderBackround(ZMap &the_map, SDL_Surface *dest, int tx, int ty)
{
	...
	//top left
	list_top_left_img.BlitSurface(nullptr, tx, ty);

	//top right
	{
		ix = tx + (w - list_top_right_img.GetBaseSurface()->w);
		list_top_right_img.BlitSurface(nullptr, ix, ty);
	}
	...
	//bottom left
	{
		...
		list_bottom_left_img.BlitSurface(nullptr, tx, iy);
	}
	...
	//bottom right
	{
		...
		list_bottom_right_img.BlitSurface(nullptr, ix, iy);
	}
...
void GMMWList::RenderEntries(ZMap &the_map, SDL_Surface *dest, int tx, int ty)
{
	...
	for(i=0,j=view_i;i<visible_entries;i++,j++)
	{
		if(j < entry_list.size())
		{
			...
			RenderEntry(the_map, dest, ix, iy + (i*MMLIST_ENTRY_HEIGHT), iw, nullptr, MMLIST_NORMAL);
	}
}
...
void GMMWList::RenderEntry(ZMap &the_map, SDL_Surface *dest, int tx, int ty, int tw, ZSDL_Surface *text, int state)
{
	...
	//left
	left.BlitSurface(nullptr, tx, ty);

	//right
	{
		ix = tx + (tw - right.GetBaseSurface()->w);
		right.BlitSurface(nullptr, ix, ty);
	}
	...
	if(text && text->GetBaseSurface())
		text->BlitSurface(0, 0, tw - 8, text->GetBaseSurface()->h, nullptr, tx + 4, ty + 4);
}
...
void GMMWList::RenderControls(ZMap &the_map, SDL_Surface *dest, int tx, int ty)
{
	...
	//buttons
	{
		...
		list_button_up_img[up_button_state].BlitSurface(nullptr, ix, iy);

		//ix = tx + (w - MMLIST_DOWN_BUTTON_FROM_RIGHT);
		iy = ty + (h - MMLIST_DOWN_BUTTON_FROM_BOTTOM);
		list_button_down_img[down_button_state].BlitSurface(nullptr, ix, iy);
	}
	...
	if(list_top_right_img.GetBaseSurface() && list_bottom_right_img.GetBaseSurface())
	{
		...
		list_scroller_img.BlitSurface(nullptr, ix, iy);
	}
}
