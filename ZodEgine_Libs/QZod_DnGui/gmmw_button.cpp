...
void GMMWButton::DoRender(ZMap &the_map, SDL_Surface *dest, int tx, int ty)
{
	...
	else
	{
		...
		non_generic_img[type][state].BlitSurface(nullptr, tx, ty);
	}
	...
	//text
	{
		...
		if(text_img.GetBaseSurface())
		{
			...
			text_img.BlitSurface(nullptr, (tx + (w >> 1)) - (text_img.GetBaseSurface()->w >> 1), ty + 3 + added_shift);
		}
	}
}
...
void GMMWButton::RenderGeneric(ZMap &the_map, SDL_Surface *dest, int tx, int ty)
{
	...
	top_left.BlitSurface(nullptr, tx, ty);

	//top right
	{
		ix = (tx + w) - top_right.GetBaseSurface()->w;
		//top_right.RenderSurface(ix, ty);
		top_right.BlitSurface(nullptr, ix, ty);
	}
	...
	//bottom left
	{
		...
		bottom_left.BlitSurface(nullptr, tx, iy);
	}
	...
	//bottom right
	{
		...
		bottom_right.BlitSurface(nullptr, ix, iy);
	}
