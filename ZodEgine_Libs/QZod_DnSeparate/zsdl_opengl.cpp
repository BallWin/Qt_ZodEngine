...
ZSDL_Surface::ZSDL_Surface()
{
	...
	size = 1.0f;
	angle = 0.0f;
    alpha = 127; //255; //!!! TODO<error>  alpha type (char)!!! max value 127! // 255
}
...
void ZSDL_Surface::SetSize(float size_)
{
    ...
    if(!use_opengl && (std::fabs(size-size_) < 1e-3)) // TODO comparision float size != size_)
	{
		if(sdl_rotozoom) SDL_FreeSurface(sdl_rotozoom);
        sdl_rotozoom = nullptr; // TODO nullptr;
		rotozoom_loaded = false;
...
{
	//unload rotozoom surface?
    if(!use_opengl && (std::fabs(angle_-angle) < 1e-3)) // TODO comparision float angle != angle_)
	{
		if(sdl_rotozoom) SDL_FreeSurface(sdl_rotozoom);
        sdl_rotozoom = nullptr; // TODO nullptr;
		rotozoom_loaded = false;
